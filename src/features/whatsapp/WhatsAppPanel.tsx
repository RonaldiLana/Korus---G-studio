...
import { User, WhatsAppIntegration, Destination, VisaType, Plan } from '../../types';
...
export const WhatsAppPanel: React.FC<WhatsAppPanelProps> = ({
  ...
  return (
    <div className="h-full flex flex-col lg:flex-row gap-0 lg:gap-6 overflow-hidden">
      {/* ─── LEFT COLUMN: QR CODE + WHATSAPP WEB ─── */}
      <div className="flex-1 flex flex-col overflow-hidden border-b lg:border-b-0 lg:border-r border-[var(--border-color)]">
        <div className="flex-1 overflow-y-auto scrollbar-hide p-6 lg:p-8 space-y-6">
          {/* Header */}
          ...
          <div>
            ...
            <WhatsAppWebEmbed isConnected={connectionStatus === 'connected'} />
          </div>
      ...
      <div className="w-full lg:w-96 bg-[var(--bg-card)]/50 border-t lg:border-t-0 lg:border-l border-[var(--border-color)] flex flex-col overflow-hidden lg:max-h-full">
        {connectionStatus === 'connected' ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-6 text-center space-y-4">
            <div className="w-16 h-16 rounded-2xl bg-[var(--bg-input)] border border-[var(--border-color)] flex items-center justify-center">
              <AlertCircle size={32} className="text-[var(--text-muted)]" />
            </div>
            <div>
              <p className="text-sm font-bold text-[var(--text-main)]">Conecte o WhatsApp primeiro</p>
              <p className="text-xs text-[var(--text-muted)] mt-2">
                Para iniciar novos processos de clientes, você precisa conectar o WhatsApp usando o QR Code ao lado.
              </p>
            </div>
          </div>
        )}
